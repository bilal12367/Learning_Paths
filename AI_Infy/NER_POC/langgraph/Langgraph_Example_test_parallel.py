...
class MyState(TypedDict, total=False):
    ...
    fields: Annotated[List[str], operator.add]
...
class LLM_Retreiver:
    ...
    def __call__(self, state: MyState):
        ...
        try:
            ...
            #     "SHENZHEN YICHANGTAI IMPORT AND EXPORT TRADE CO., LTD",
            #     "SHENZHEN LEHEYUAN TRADING CO, LTD"
            # ],
            # "date": "09/12/2019",
            # "contract_type": "Supply Contract",
            # "summary": "This is a framework agreement where the buyer entrusts SHENZHEN YICHANGTAI IMPORT AND EXPORT TRADE CO., LTD or SHENZHEN LEHEYUAN TRADING CO, LTD to purchase products from the seller. The contract is valid for 5 years and governed by the law of the People\u2019s Republic of China. Payment will be made via Letter of Credit or T/T.",
            # "missing_fields": [],
            # "file_name": "LohaCompanyltd"
            # }
        except Exception as e:
        ...
        if len(missing_fields) == 0:
            ...
            return {"token_count": tokens, "fields": [],"input_query": "Parties Involved, Contract Type, Contract Date, Summary", "max_retries": 3, "rel_docs": [],"resp_obj": resp}
        else:
...
graph.add_node(Nodes.COMBINATOR.value, combinator, defer=True)
...
def file_looper(state: MyState):
    print("File Looper: ")
    print(state.get('file_names'))
    resp_obj = state.get('resp_obj')
    print(resp_obj)
    if state.get('error'):
        return 'end'
    
    if len(resp_obj.get('missing_fields')) != 0 and state.get("max_retries") > 0:
        print("In retrieval")
        print(state.get('fields'))
...
graph.add_edge(Nodes.SPLIT_DOCS.value, Nodes.NER_Extraction.value)
...
graph.add_conditional_edges(Nodes.LLM_Retreival.value, 
    file_looper,
    {
        ...
        "end": END,
        "combinator": Nodes.COMBINATOR.value
    }
)
